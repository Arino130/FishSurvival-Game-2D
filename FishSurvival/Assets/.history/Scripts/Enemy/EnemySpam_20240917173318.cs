...
public class EnemySpam : MonoBehaviour
{
    ...
    private IEnumerator spamObject()
    {
        while (true)
        {
            ...
            GameObject fishItem = Instantiate(fishLevel1, new UnityEngine.Vector3(transform.position.x, transform.position.y, -1f),
            fishLevel1.transform.rotation, enemyFolder);
            switch (spamPosition)
            {
