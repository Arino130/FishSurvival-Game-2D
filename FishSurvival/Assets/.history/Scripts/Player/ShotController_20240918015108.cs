...
public class ShotController : MonoBehaviour
{
    ...
    void Attack()
    {
        ...
        originalPosition = gunBarrel.transform.position;
        StartCoroutine(MoveBackAndForth());
    }

    private IEnumerator MoveBackAndForth()
    {
        Vector3 moveBackOffset = new Vector3(0f, 1f, 0f);
    ...
    {
        Vector3 startPosition = gunBarrel.transform.localPosition;
        float timeElapsed = 0;

        while (timeElapsed < duration)
        {
            gunBarrel.transform.localPosition = Vector3.Lerp(startPosition, target, timeElapsed / duration);
            timeElapsed += Time.deltaTime;
            yield return null;
        }

        gunBarrel.transform.localPosition = target;
    }
}