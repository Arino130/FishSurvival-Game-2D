...
public class StatisticsPlayer : MonoBehaviour
{
    ...
    private IEnumerator MoveAndFadeCoroutine()
    {
        while (elapsedTime < fadeDuration)
        {
            ...
            float newX = Mathf.Lerp(startPosition.x, startPosition.x + moveDistance, progress);
            textCoinMessageValue.transform.position = new Vector3(newX, textCoinMessageValue.transform.position.y, textCoinMessageValue.transform.position.z);
            if (spriteRenderer != null)
            {
        ...
        textCoinMessageValue.transform.position = new Vector3(startPosition.x + moveDistance, textCoinMessageValue.transform.position.y, textCoinMessageValue.transform.position.z);
        if (spriteRenderer != null)
        {
